...
| `BrokeModeBanner` / `BrokeModeDialog` (broke mode) | ✅ | `modes_cards.dart`: banner on Home and the budget screen while it runs, entry card and dialog on the budget screen. Kotlin's plan arithmetic (`brokeModePlan`), online and read back. |
| `SavingsChallengeCard` + share | ✅ | Card on Home while it runs; entry, dialog (suggested cap, 7/14/30) and stop on the budget screen; share through `share_plus`. |
...
| `InventoryScreen`: categories, low-stock banner, expiring soon, edit, shortages, search, empty states | ✅ | `pantry_view.dart`: the all/shortages tabs, search with its empty state, 9 category chips (Kotlin's rule: stored category, else the name), per-item picture, fullness bar, expiry, −/+, an edit sheet with delete, and add with unit/category/expiry. The expiring section's "اقتراح وصفة" goes to the chat. Kotlin's low-stock banner auto-replenish is Flutter's automatic shortage→list. Not ported: the Amazon "buy this" chips (affiliate) and the inventory camera, which waits on camera modes. |
| `ShoppingListScreen`: budget header, priority chips, grocery suggestions, affiliate footer | ✅ | `shopping_list_view.dart`: basket vs remaining budget (unknown ≠ zero) and the over-budget warning; "تعبئة ذكية" (`grocery_suggestions` + `estimate_price` for up to 5 lines, on the tap rather than on open as Kotlin does); WhatsApp share; priority chips; lines with priority tag, price, store, days left, tick ("تم شراء"), delete; the add sheet with price and store. 🚫 The Amazon alternative and the affiliate footer (affiliate). |
| `PantryShoppingScreen` (tabs) | ✅ | Household tab. |
| `PharmacyScreen`: expiry tracker, grid, smart add, dose picker, refill, stats, family pharmacy | ⚠️ | `pharmacy_view.dart`: adherence and monthly cost (Kotlin's max of pharmacy spending vs box prices), the counts, the expired warning, the expiry tracker, today's doses (take/snooze), each medicine card (status stripe, schedule, family member, expiry/stock badge, days of supply or "الكمية محتاجة تأكيد", تناول جرعة, تجديد الطلب, فاضل قد إيه فعلاً؟ with units per dose, delete), the add form (units, doses/day, preset and picked times with Kotlin's suggested spread, expiry, price, active ingredient, dosage, chronic, family member, category), and add-by-talking through the chat. The model now reads/writes `price`, `is_recurring`, `family_member_id` and `active_ingredient`, and leaves `units_per_dose` null when unknown instead of writing 1. Missing: the family pharmacy tab (admins), the grid layout toggle, the box scan (camera modes), and Kotlin's exact-alarm prompt (N/A: reminders are server push). |
