-- مشاعر فويس تليجرام من "اللحظة" مش من تخمين النص (٢٠٢٦-٠٩-١٤).
--
-- `_shared/zadVoice.ts` بقى مصدر شخصية الصوت الواحدة (MOMENT_EMOTIONS). البوت بيقرا
-- `moment` من طلب realtime_push ويحوّله لمشاعر: budget_90 = قلقانة، budget_100 = زعلانة،
-- critical_insight = قلقانة. من غيره الاتنين كانوا بيتقروا "قلقانة" من كلمات العنوان.
--
-- نفس تعريفي 20260914001000 بالحرف؛ الفرق الوحيد حقل 'moment'.
    ...
    IF threshold_to_fire IS NOT NULL THEN
        ...
        PERFORM net.http_post(
            ...
            body:=jsonb_build_object('user_id', NEW.user_id, 'title', push_title, 'body', push_body,
                                     ...
                                     'moment', CASE WHEN threshold_to_fire >= 100 THEN 'budget_100' ELSE 'budget_90' END),
            timeout_milliseconds:=15000
        );
    END IF;
