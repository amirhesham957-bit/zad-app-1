...
/// The way in from Home: what is running, what it costs, what comes next.
class SubscriptionsEntryCard extends ConsumerWidget {
  /// Creates the card.
  const new({super.key});
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final running = view.active.toList();
    final soonest = running.isEmpty
        ? null
        : running.first.nextRenewalFrom(view.today);

    final total = '${_money(view.monthlyTotal)} $currency'.trim();
    final summary = '${running.length} شغّالين · $total في الشهر';
    // `items` is sorted soonest first, so the first running row is the next
    // charge.
    final subtitle = running.isEmpty
        ? 'ضيف اللي بيتجدد كل شهر، وزاد يحجزله من ميزانيتك.'
        : soonest == null
        ? summary
        : '$summary\nالجاي: ${running.first.title} '
              '${_whenLabel(soonest, view.today)}';

    return ZadCard(
      onTap: () => showSubscriptionsScreen(context),
      child: Row(
        children: <Widget>[
          const Icon(ZadIcons.obligation, color: ZadColors.green700),
          const SizedBox(width: ZadSpacing.md),
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: <Widget>[
                const Text('الاشتراكات والفواتير', style: ZadType.titleSmall),
                const SizedBox(height: ZadSpacing.xs),
                Text(
                  subtitle,
